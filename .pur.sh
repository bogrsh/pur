            ...
            if root.tag == 'rss':
                for i in root.findall('./channel/item'):
                    items.append(FeedItem(
                        ...
                        date=self.parse_date(i.findtext('pubDate')),
                        source=name
                    ))
                ...
                for e in root.findall('a:entry', ns):
                    ...
                    items.append(FeedItem(
                        ...
                        date=self.parse_date(e.findtext('a:updated', namespaces=ns)),
                        source=name
                    ))
            return items
...
class CursesUI:
    ...
    def draw(self, stdscr):
        ...
        for i in range(h-1):
            ...
            date = it.date.strftime("%m-%d %H:%M")
            line = f"{mark} {date} [{it.source}] {it.title}"[:w-1]
